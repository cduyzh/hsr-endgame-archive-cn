...
- 投稿凭证：投稿成功后服务端下发 `ownerToken`（`own_<48 hex>`），写回本机 localStorage，**`/me` 页面**可按 token 反查该用户提过的所有 `submission_reviews` + `runs`，查看审核进度（pending/approved/rejected/withdrawn）、忘记某条凭证或一键清空。按状态给不同动作：待审 / 已通过可**撤回**；已通过可**编辑并重新提交**（产生一条带 `revises_id` 的待审**修订**，管理员通过之前公开档案仍展示上一次通过的记录，通过后原地更新那条 `runs`、不新增记录）；已驳回可**编辑并重新提交**（就地重提同一条记录）、**隐藏展示**、**删除记录**（硬删，`runs` + `submission_reviews` 一起移除、不可恢复，页内两步确认）；已撤回可**隐藏展示**。隐藏是写在 `submission_reviews.hidden` 上的**服务端状态、跨设备生效**，`/me` 默认过滤掉已隐藏的整条家族并回 `hiddenCount`，页顶「已隐藏 N 条 · 查看」可展开并取消隐藏——所以隐藏可逆；删除则严格只开放给 `rejected`（已通过记录不允许作者自行抹掉公开档案）。
...
- **生产库里有本仓库不认识的表与列（归属已查实：本站自己的历史遗留）**（2026-09-14 复核）：除本仓库那 8 张表外，库里还有 `hsr_shared` schema（11 张表）、`public.archive_stage_sources`（FK 指向 `hsr_shared.data_releases` 与 `public.stages`）、`public.archive_schema_migrations`，以及 `characters.source_id text` 与 `characters.recommended_lightcone_ids jsonb not null default '[]'` 两列。归属结论与取证过程见 [`docs/wiki/04-生产验证缺陷清单`](./docs/wiki/04-生产验证缺陷清单.md) 的「D16」：`archive_schema_migrations` 只有 `001_hsr_shared.sql` 与 `002_character_lightcone_recommendations.sql` 两条、同为 2026-07-22 同一秒应用（本仓库 07-14 开站），两个迁移文件从未进过 git；`hsr_shared` 与 `archive_stage_sources` **全部 0 行**（那条共享数据管道从未启用）；`recommended_lightcone_ids` 是**被 `lightcone-pairs.json` 取代的弃用「角色推荐光锥」特性**，95/98 行有值，缺的三行是两个变体皮 + 新增的真珠。影响面：`pnpm seed:archive` 的 upsert 只写 `name/path/element/rarity/limited`，**不会覆盖**那两列，但新插入的角色行会落默认值（`[]` / `NULL`），这与那三个本来就无解的行同类，不是本轮灌库造成的破口。处置：**保持不动**；要收口先把这两个迁移文件补进仓库留痕、再单独决定删或留（`DROP` 属破坏性操作，未获批准前不要执行），也不要往 `schema.sql` 里补建（那会把已废弃的结构当成本站能力宣传）。
