...
- **投稿自助端点共用两个小工具**：`sanitizeOwnerToken(value)`（非字符串 / 空 / 超 200 字符一律非法）与 `readSubmissionId(event)`（`netlify.toml` 重写成 `/.netlify/functions/<name>/:id`，取路径末段，`?id=` 优先）。撤回 / 隐藏 / 删除 / 修订四个端点都用它们——凭证校验口径不一致就等于鉴权口径不一致。
