...
| `submissions.ts`          | `/api/submissions`              | POST 投稿，非法 JSON 返回 `400`，缺字段返回 `400 {missing}`，**值域违规返回 `400 {message, missing:[], violations}`**（`checkSubmissionRules`），**查重命中返回 `409 {message, duplicate}`**，入队 `pending` 返回 `202`，**同时下发 `ownerToken`（`own_<48 hex>`）到响应体**       |
...
| `admin-submissions.ts`    | `/api/admin/submissions`        | GET 审核列表（需鉴权），支持 `status` 过滤，白名单为 `pending`/`approved`/`rejected`/**`withdrawn`**/`all`（`withdrawn` 此前缺席，`?status=withdrawn` 一律 400，撤回态在审核台侧隐形）；带 `revisesId` 供审核台认出修订，不带 `hidden` / `owner_token` |
| `admin-submissions-id.ts` | `/api/admin/submissions/:id`    | PATCH 审核（通过/驳回/退回），需鉴权。**通过前先用 `checkSubmissionRules` 复校 payload**（脏值 400，避免绕过前端入队的记录被发布进公开 `runs`），再确认 `bossId` 在 `stages` 表**或**远程快照里至少存在一处——两处都没有就 `400` 拒绝通过，**不再造 `name=bossId` 的最小占位 stages 行**（那种行没有任何端点能删）。`stages` → `runs` → `run_units` → 置 `approved` → 改审核行这一串写包进**一笔 neon 非交互式事务**（`sql.transaction([查询数组])`），任一步失败整体回滚并返回 `502`；**写入 `runs.owner_token` 供用户后续检索**；写入的目标行是 `revisesId ?? 投稿 id`，所以修订通过后原地更新原公开记录。驳回/退回分支保持按投稿 id 更新，修订没有 `runs` 行因此不会碰到父记录 |
...
- **`validateSubmission(payload)` 返回 `{ missing, violations }`**：`missing` 是存在性缺失的字段名（前端 `describeSubmissionFailure()` 会用 `submissionFieldLabels` 翻成中文），`violations` 是值域违规（`{field, message}`，message 已是中文）。值域判定复用 `src/services/submissionRules.ts` 的 `checkSubmissionRules()`——模式枚举、分类是否属于当前模式与阶段、阶段 id 与赛季/模式是否一致、成本 `0–48`、`4 角色 + 4 光锥`、视频域名白名单、标记取值、0 轮分类要求轮次 0、`as` 分数上限。此前这些规则**只有前端一份**，绕过页面直 POST 就能把 `mode=bogus`、`cost=999` 的脏记录送进队列并在通过后进公开 `runs`。两个入口的响应 shape：缺字段 `400 {message:"缺少必要字段", missing}`，值域违规 `400 {message:<第一条中文说明>, missing:[], violations}`（`missing` 刻意留空，否则前端会把它误翻成「缺少必要字段」）。改规则只需改 `submissionRules.ts` 一处，`submissionValidation.ts` 与这里都从它取。
...
- **审核通过时补 `stages` 行**：`runs.boss_id` 是 FK 引用 `stages(id)`，库内 `stages` 默认空。`admin-submissions-id.ts` 的 approved 分支在写 `runs` 之前先查一次 `select id from stages where id = <bossId>`，再 `_staticSnapshot.getStaticBossMap()` 拉一次远程 `static.nanoka.cc`（与前端 `staticArchiveConfig.ts` 共用纯计算模块 `src/services/staticBossSnapshot.ts`）：
  - 快照命中 → 在事务里 `insert ... on conflict (id) do update set` 写入 13 列完整数据（name/variant_name/subtitle/hp/speed/toughness/weakness/resist/clears/mechanic/stage_buffs/banner_tone），供服务端统计/导出也能读到真实数值；
  - 快照没有但库里已有该 `stages` 行（历史赛季）→ 跳过 upsert，直接发布；
  - **两处都没有 → `400` 拒绝通过**，提示先跑 `POST /api/admin/sync-stages`。旧实现在这里降级写一行 `name=bossId` 的最小占位，而那一行**没有任何端点能删**，写入失败时就永久留在库里，因此已删除。
  冷启动内多次 PATCH 复用同一份快照，Netlify 冷启动间内存不持久所以每次冷启动都重新拉。
- **审核写入必须是一笔事务**：`stages` upsert → `runs` upsert（先 `status='pending'`）→ `run_units` 删旧插新 → 置 `approved` → 改 `submission_reviews`，全部放进 `sql.transaction([查询数组])` 一次提交，失败返回 `502` 中文提示。**neon 的 `transaction()` 是非交互式的**：只能传查询数组或返回数组的**非 async** 函数，不能在回调里 `await`（`sql.unsafe()` 在 1.x 也只产出片段、不会自己执行，要 `await sql\`${sql.unsafe(stmt)}\``）。旧实现逐条 `await`、整段无事务，占位 stages 行在 `runs` insert 之前就落库，所以**光加 try/catch 只是把 500 变成 502、回收不了已提交的行**。
...
- **`runs.status` 与 `runs.mode` 现在带 CHECK 约束**（`netlify/migrations/2026-09-14_runs_status_and_mode_check.sql`，同一份语句也在 `schema.sql` 末尾的幂等迁移区；2026-09-14 已在生产库执行并实测拒绝脏值）：`status in ('pending','approved','rejected','withdrawn')`、`mode in ('moc','pf','as','aa')`。取值集合是**按代码实际写入核对**的（`withdrawn` 来自 `submissions-withdraw.ts` 按 `runs.id` 精确改那一条），照抄审核队列的四态而不核对会把线上写挂。加约束前务必先 `select status, count(*) from runs group by status` 核对存量。
  - **`category` 有意不加约束**：根 `AGENTS.md` 的口径是「`runs.category` 是开放 text、无枚举约束，新增取值不需要迁移」，分档随模式与阶段变化，值域由 `src/services/submissionRules.ts` 在入队与发布前把关。
  - ⚠️ **生产库的结构比本仓库 `schema.sql` 多**（2026-09-14 实测）：另有 `hsr_shared` schema（`data_releases` / `sync_runs` / `release_files` / `characters` / `lightcones` / `monsters` / `monster_values` / `endgame_seasons` / `endgame_stages` / `stage_monsters` / `current_release`）、`public.archive_stage_sources`、`public.archive_schema_migrations`，以及 `characters.source_id` 与 `characters.recommended_lightcone_ids jsonb not null default '[]'`。**本仓库没有任何代码读写这些结构**（grep 零命中），来源待确认；`pnpm seed:archive` 的 upsert 只写 `name/path/element/rarity/limited`，不会覆盖它们，但**新插入的角色行 `recommended_lightcone_ids` 会落到默认 `[]`、`source_id` 为 NULL**（2026-09-14 的「真珠」即如此）。动这张表前先确认这些列归谁用。
