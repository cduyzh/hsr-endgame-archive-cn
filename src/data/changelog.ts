...
export const changelogEntries: ChangelogEntry[] = [
	{
		...
		title: "新角色数据与审核台修订",
		items: [
			{tag: "data", text: "投稿时可选到 4.5 当期新五星「真珠」，选中后自动带出她的专武光锥《献给明日的色彩》，成本按限定五星口径合计"},
			{tag: "fix", text: "角色名「银狼LV.999」不再显示成带尖括号的原始排版标记，记录列表、环境统计与投稿预览三处同步修正"},
			{tag: "improve", text: "审核台的敌方阶段改按「4.5 · 将杀」这样的展示词回显（悬浮可看原始阶段 id），与「我的投稿」口径一致"},
			{tag: "fix", text: "作者已撤回的投稿在审核台不再错显成「已驳回」：单独显示「已撤回」并新增该状态的筛选页签，同时不再给出「通过并发布」与「驳回」按钮，避免把作者主动撤下的记录重新发回公开档案；需要重新上架时用「退回待审」"},
			{tag: "fix", text: "把一条被驳回的记录改判为通过时，不再把上一次的驳回原因一起带回记录；管理员这次确实新写过的备注照常保存"},
			{tag: "improve", text: "投稿在服务器侧也按与表单相同的规则校验（模式、记录分类、成本区间、4 角色 + 4 光锥、视频域名、标记取值），绕过页面直接提交的脏数据会在入队时被拦下并给出中文原因，不会再进入公开档案"},
			{tag: "improve", text: "审核通过改为一笔事务写入：任何一步失败都整体回滚并提示稍后重试，不会再留下删不掉的半成品数据；敌方阶段既不在库里也不在数据快照中时直接拒绝通过，而不是造一行占位阶段"},
			{tag: "improve", text: "审核台登录框不再预填 admin 这个默认账号（生产配的是别的账号，预填值必然登录失败），改为留空并提示账号来自部署环境配置"},
		],
	},
	{
		version: "0.9.1",
