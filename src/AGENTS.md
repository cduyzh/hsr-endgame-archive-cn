...
- **面向用户的界面不直接印原始 id**：模式名查 `config.modes[].label`、角色与光锥名查 `config.units[].name`（`buildSubmissionRoster()` 已封装）、阶段展示词查 `runUtils.ts` 的 `stageKeyLabels` / `stageLabelOf(bossId)`（`4.5-aa-k3` → 「K3」，完整首领名放 `title`）。页面要展示这些名字时得先 `useArchiveStore().loadConfig()`（幂等），`MySubmissionsView.vue` 就是这么取中文名的；配置没到位时退回原 id，不要留空。
